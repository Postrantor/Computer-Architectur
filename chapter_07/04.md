...
> 鉴于绩效模型的这些见解，TPU 架构师接下来评估了一个替代性和假设的 TPU，如果他们有超过 15 个月的时间，他们可能会在相同的过程技术中设计。更具侵略性的逻辑合成和块设计可能会使时钟速率提高 50％。工程师发现，设计用于 GDDR5 内存的接口电路(K80 使用)将使重量存储器带宽提高超过 5 倍。如[图 7.18](#_ bookmark346)所示，将时钟速率提高到 1050 MHz，但没有帮助记忆，几乎没有变化的性能。如果时钟以 700 MHz 的速度保留，但它使用 GDDR5 代替重量存储器，则性能增加了 3.2，甚至考虑了主机 CPU 在修订后的 TPU 上调用 DNN 的开销。两者都不会进一步提高平均表现。
