...
> 嵌入式计算机存在于日常机器中：微波炉、洗衣机、大多数打印机、网络交换机和所有汽车。短语 _物联网_ (IoT) 是指连接到互联网的嵌入式计算机，通常是无线连接。当增加传感器和执行器时，物联网设备会收集有用的数据并与物理世界交互，从而产生各种各样的“智能”应用，例如智能手表、智能恒温器、智能扬声器、智能汽车、智能家居、智能电网、 和智慧城市。
...
> [!note] : 实时性
> 这里对实时性的问题给出一个界定的条件，限定特定任务的平均时间以及超过某个最大时间的实例数。
> 并且指出，实时性往往过高的依赖于应用程序。
