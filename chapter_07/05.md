...
> FPGA 中重编程性的成本比定制芯片慢。机器学习的评分使用两种形式的并行性来克服这种不利地位。首先是具有与应用程序中可用管道并行性匹配的管道。对于排名，极限为每个阶段 8μs。并行性的第二版是很少见的_ multiple 指令流<ub>，</sub>单个数据流(Misd)_并行性，其中大量独立的指令流在单个文档上并行在单个文档上并行运行。
