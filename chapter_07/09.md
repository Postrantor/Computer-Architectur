...
> [图 7.49](#_ bookmark383)给出了每个模具相对推理性能的底线，包括两个加速器的主机服务器开销。回想一下，当工程师不知道将运行的程序的实际组合时，使用几何均值。但是，对于此比较，我们_do_知道混合物([图 7.41](#_ bookmark374))。使用实际混合物的[图 7.49](图 7.49](#_ bookmark383)的最后一列中的加权平均值使 GPU 高达 1.9 倍，而 TPU 的速度是 CPU 的 29.2 倍，因此 TPU 的速度是 15.3 倍 GPU。
