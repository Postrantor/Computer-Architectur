...
> ##多处理器和并行处理的历史记录([[第 5 章](#_bookmark213)和 [appendices f](#_bookmark595)，[g](#_bookmark686) 和 [i](#_bookmark736))
...
> SIMD 最适合处理阵列以进行循环；因此，要在 simd 中有大量并行性的机会，必须有大量的数据或 *data parallealism*。在案件陈述的情况下，SIMD 是最弱的，每个执行单元必须在其数据上执行不同的操作，具体取决于其拥有的数据。具有错误数据的执行单元被禁用，以便可以继续进行适当的单位。这种情况基本上以 1/_n_ 的性能运行，其中 *n* 是案例数。
...
> 尽管 SIMD 计算机已作为通用替代方案退出市场，但这种架构风格将继续在专用设计中发挥作用。许多特殊用途的任务是高度数据并行的，需要一组有限的功能单元。因此，设计人员可以构建对某些操作的支持，以及功能单元之间的硬连线互连路径。这种组织通常称为*阵列处理器*，它们对图像和信号处理等任务很有用。
...
> 第二个钟声预测涉及 1995 年运送的超级计算机中数据流的数量。丹尼·希尔斯(Danny Hillis)认为，尽管具有少量数据流的超级计算机可能是最畅销的人，但最大的多处理器将是具有许多数据流的多处理器，并且这些将执行大部分计算。Bell Bet Hillis 认为，在 1995 年日历年的最后一个季度，将使用很少的数据流(≤100)而不是许多数据流(≥1000)在多处理器中发货。这个赌注仅涉及超级计算机，该超级计算机被定义为耗资超过 100 万美元的多处理器，用于科学应用。该赌注定义为持续的 mflops 是每个* month*的浮点操作的数量，因此多处理器的可用性会影响其评分。
...
> 尽管在 1960 年代和 1970 年代使用多个处理器建造了非常大的大型机，但直到 1980 年代，多处理器才变得非常成功。Bell [1985]建议关键是微处理器的尺寸较小，使内存总线可以替换互连网络硬件，并且便携式操作系统意味着多处理器项目不再需要新操作系统的发明。贝尔在他的论文中定义了术语 *mul-tipRocessor* 和 *multicomputer*，并为构建较大尺度多处理器的两种不同方法设定了舞台。
...
> 在开发替代连贯协议的开发中爆炸了，Archibald and Baer [1986]提供了良好的调查和分析，并参考了原始论文。[图 M.2](#_bookmark912) 总结了几个窥探缓存协调协议，并显示了一些使用或正在使用该协议的多处理器。
...
> 使用虚拟内存在不同机器之间实现共享地址空间的概念最早出现在 1988 年 Kai Li 的 Ivy 系统中。随后有论文探讨了硬件支持问题、软件机制和编程问题。阿姆扎等。[1996] 描述了一个使用新的一致性模型建立在工作站上的系统，Kontothanassis 等人。[1997] 描述了一种使用远程写入的软件共享内存方案，Erlichson 等人。[1996] 描述了使用共享虚拟内存来构建使用 SMP 作为节点的大规模多处理器。关于多处理器和多台计算机的信息量几乎是无限的：会议、期刊论文，甚至书籍似乎出现得比任何一个人吸收这些想法的速度都快。毫无疑问，这些论文中的许多将被忽视——与过去不同。大多数主要的体系结构会议都包含有关多处理器的论文。一年一度的会议，超级计算 _XY_(其中 _X_ 和 _Y_ 是年份的最后两位数字)汇集了用户、架构师、软件开发人员和供应商，会议记录以书籍、CD-ROM 和在线形式发布(参见 [  _www.scXY.org_](http://www.scXY.org/)) 表格。_并行和分布式计算杂志_和_IEEE Transactions on Parallel and Distributed Systems_这两个主要期刊包含有关并行处理各个方面的论文。以下参考资料中包含几本侧重于并行处理的书籍，其中 Culler、Singh 和 Gupta [1999] 是最新的大型著作。多年来，NASA 艾姆斯研究中心的 Eugene Miya 收集了并行处理论文的在线参考书目。
